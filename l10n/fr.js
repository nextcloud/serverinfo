OC.L10N.register(
    "serverinfo",
    {
    "System" : "Système",
    "Database configuration" : "Configuration de la base de données",
    "The database configuration could not be checked." : "La configuration de la base de données n'a pas pu être vérifiée.",
    "These checks only cover MySQL, MariaDB and PostgreSQL." : "Ces vérifications ne couvrent que MySQL, MariaDB et PostgreSQL.",
    "All database checks pass." : "Toutes les vérifications de la base de données passent avec succès.",
    "_%n database check is failing. {link}_::_%n database checks are failing. {link}_" : ["%n vérification de la base de données échoue. {link}","%n vérifications de la base de données échouent. {link}","%n vérifications de la base de données échouent. {link}"],
    "Show the findings…" : "Afficher les résultats…",
    "Unknown" : "Inconnu",
    "%1$d days, %2$d hours, %3$d minutes, %4$d seconds" : "%1$d jours, %2$d heures, %3$d minutes, %4$d secondes",
    "%1$d hours, %2$d minutes, %3$d seconds" : "%1$d heures, %2$d minutes, %3$d secondes",
    "Monitoring" : "Surveillance",
    "Monitoring app with useful server information" : "Application de surveillance avec les informations utiles du serveur",
    "Provides useful server information, such as CPU load, RAM usage, disk usage, number of users, etc." : "Fournit des informations utiles sur le serveur, telles que la charge du processeur, l'utilisation de la RAM, l'utilisation du disque, le nombre d'utilisateurs, etc.",
    "{0}% of all users" : "{0}% de tous les utilisateurs ",
    "Active users" : "Utilisateurs actifs",
    "Last hour" : "Dernière heure",
    "Last 24 Hours" : "Dernières 24 heures",
    "Last 7 Days" : "7 derniers jours",
    "Last 30 Days" : "30 derniers jours",
    "System cron" : "Cron système",
    "Webcron" : "Webcron",
    "AJAX (not recommended)" : "AJAX (non recommandé)",
    "Background jobs" : "Tâches d'arrière-plan",
    "Mode" : "Mode",
    "Last run" : "Dernière exécution",
    "Never" : "Jamais",
    "Latest runs" : "Dernières exécutions",
    "No background job has run yet." : "Aucune tâche d'arrière-plan n'a été exécutée pour le moment.",
    "Slowest jobs" : "Tâches les plus lentes",
    "The slow jobs statistics are not available yet. They are collected by a background job and appear after its next run." : "Les statistiques sur les tâches lentes ne sont pas disponibles pour le moment. Elles sont collectées par une tâche en arrière-plan et apparaîtront après la prochaine exécution de celle-ci.",
    "_Latest failures (last %n day)_::_Latest failures (last %n days)_" : ["Derniers échecs (%n dernier jour)","Derniers échecs (%n derniers jours)","Derniers échecs (%n derniers jours)"],
    "_No background job failed in the last %n day._::_No background job failed in the last %n days._" : ["Aucune tâche d'arrière-plan n'a échoué au cours du %n dernier jour.","Aucune tâche d'arrière-plan n'a échoué au cours des %n derniers jours.","Aucune tâche d'arrière-plan n'a échoué au cours des %n derniers jours."],
    "Load" : "Charge",
    "CPU info not available" : "Informations CPU non disponibles",
    "Current usage" : "Utilisation actuelle",
    "Threads" : "Fils d’exécution",
    "Load average" : "Moyenne de la charge système",
    "The number of CPU threads is unknown, so the load cannot be shown as a percentage." : "Le nombre de threads CPU étant inconnu, la charge ne peut pas être affichée en pourcentage.",
    "Queries per second" : "Requêtes par seconde",
    "Transactions per second" : "Transactions par seconde",
    "Current connections" : "Connexions actuelles",
    "connections" : "connexions",
    "Database latency" : "Latence de la base de données",
    "Passing" : "Passe",
    "Skipped" : "Ignoré",
    "Failing" : "Échoue",
    "Alert" : "Alerte",
    "Notice" : "Information",
    "Warning" : "Avertissement",
    "Value" : "Valeur",
    "Setting" : "Paramétrage",
    "restart required" : "redémarrage requis",
    "Note" : "Note",
    "Show config snippet" : "Afficher l'extrait de configuration",
    "Documentation ↗" : "Documentation ↗",
    "Database" : "Base de données",
    "Type:" : "Type :",
    "Version:" : "Version :",
    "Size:" : "Taille :",
    "Connected. Server version: {version}" : "Connecté. Version du serveur : {version}",
    "Database connection" : "Connexion à la base de données",
    "By default the checks use the same connection as Nextcloud. Give a separate connection here to run them as a user with the rights Nextcloud does not need, for example to read the full server configuration. Leave the host empty to use the default connection." : "Par défaut, les vérifications utilisent la même connexion que Nextcloud. Indiquez ici une connexion séparée pour les exécuter avec un utilisateur qui a des droits dont Nextcloud n'a pas besoin, par exemple pour lire la configuration complète du serveur. Laissez l'hôte vide pour utiliser la connexion par défaut.",
    "Host" : "Hôte",
    "Port" : "Port",
    "User" : "Utilisateur",
    "Password (stored, leave empty to keep)" : "Mot de passe (stocké, laissez vide pour le conserver)",
    "Password" : "Mot de passe",
    "Database name" : "Nom de la base de données",
    "Driver" : "Pilote",
    "Cancel" : "Annuler",
    "Test connection" : "Tester la connexion",
    "Save" : "Enregistrer",
    "Configuration snippet" : "Extrait de configuration",
    "Add the following to your {file}, then restart the database." : "Ajoutez ce qui suit dans votre {file} puis redémarrez la base de données.",
    "Copied" : "Copié",
    "Copy to clipboard" : "Copier dans le presse-papiers",
    "Could not copy to clipboard. Select the text above manually instead." : "Impossible de faire la copie dans le presse-papiers. À la place, sélectionnez le texte ci-dessus manuellement.",
    "Close" : "Fermer",
    "{used} of {total} used" : "{used} de {total} utilisé",
    "Used" : "Utilisé",
    "Available" : "Disponible",
    "Disk" : "Disque",
    "Files" : "Fichiers",
    "Storages" : "Stockages",
    "Free space" : "Espace libre",
    "Mount:" : "Montage :",
    "Filesystem:" : "Système de fichiers :",
    "Available:" : "Disponible :",
    "Used:" : "Utilisé :",
    "Class" : "Classe",
    "Status" : "Statut",
    "Started" : "Démarré",
    "Duration" : "Durée",
    "Peak memory" : "Pic de mémoire",
    "Run ID" : "ID de l’exécution",
    "Server ID" : "ID du serveur",
    "Process ID" : "ID du processus",
    "Details about {job} from {time}" : "Détails de {job} ({time})",
    "Job" : "Tâche",
    "When" : "Quand",
    "Details" : "Détails",
    "Succeeded" : "Succès",
    "Failed" : "Échec",
    "Crashed" : "Plantage",
    "Running" : "En cours d’exécution",
    "RAM usage" : "Utilisation RAM",
    "Swap usage" : "Utilisation SWAP",
    "Memory" : "Mémoire",
    "RAM info not available" : "Informations RAM non disponibles",
    "Total" : "Total",
    "Swap used" : "SWAP utilisée",
    "External monitoring API" : "API de surveillance externe",
    "Endpoint URL" : "URL du point de terminaison",
    "Configuration" : "Configuration",
    "Output in JSON" : "Sortie en JSON",
    "Skip apps section" : "Ignorer la section des applications",
    "Including the apps section sends an external request to the app store" : "Inclure la section des applications envoie une requête externe vers l'App Store",
    "Skip server update" : "Ignorer la mise à jour du serveur",
    "Authentication" : "Authentification",
    "This token was generated in your browser and is not stored until you run the command below. Send it in the {header} header with every request." : "Ce jeton a été généré dans votre navigateur et n'est pas enregistré tant que vous n'avez pas exécuté la commande ci-dessous. Envoyez-le dans l'entête {header} avec chaque requête.",
    "Command to store the token" : "Commande pour enregistrer le jeton",
    "Request header" : "Entête de requête",
    "Network" : "Réseau",
    "Hostname" : "Nom de l’hôte",
    "Gateway" : "Passerelle",
    "DNS" : "DNS",
    "Status:" : "État :",
    "Speed:" : "Vitesse :",
    "Duplex:" : "Duplex :",
    "MAC:" : "MAC :",
    "IPv4:" : "IPv4:",
    "IPv6:" : "IPv6:",
    "OPcache is not loaded." : "OPcache n'est pas chargé.",
    "OPcache is disabled." : "OPcache est désactivé.",
    "Nextcloud is not permitted to read the OPcache status (\"opcache.restrict_api\")." : "Nextcloud n'a pas la permission de lire le statut d'OPcache (« opcache.restrict_api »).",
    "OPcache status is unavailable." : "Le statut d'OPcache n'est pas disponible.",
    "{used} of {total}" : "{used} sur {total}",
    "Interned strings" : "Chaînes internées",
    "Keys" : "Clés",
    "{used} of {max}" : "{used} sur {max}",
    "Disabled" : "Désactivé",
    "Enabled, {used} of {total} buffer used" : "Activé, {used} utilisés sur un tampon de {total}",
    "OPcache" : "OPcache",
    "Hit rate" : "Taux de réussite",
    "Cached scripts" : "Scripts mis en cache",
    "These numbers describe the PHP process handling this request. Other FPM pools or the CLI keep their own OPcache." : "Ces valeurs décrivent le processus PHP qui traite cette requête. Les autres pools FPM ou la CLI disposent de leur propre OPcache.",
    "Revalidate frequency:" : "Fréquence de revalidation :",
    "seconds" : "secondes",
    "Validate timestamps:" : "Validation des horodatages :",
    "Yes" : "Oui",
    "No" : "Non",
    "OOM restarts:" : "Redémarrages par manque de mémoire :",
    "Last restart:" : "Dernier redémarrage :",
    "JIT:" : "JIT :",
    "PHP extensions" : "Extensions PHP",
    "Extension" : "Extension",
    "Unable to list extensions" : "Impossible de lister les extensions",
    "Unlimited" : "Illimité",
    "{count} loaded" : "{count} chargées",
    "PHP" : "PHP",
    "Version" : "Version",
    "Memory limit" : "Limite de mémoire",
    "Max execution time:" : "Temps d’exécution maximal :",
    "Upload max size:" : "Taille de téléversement maximale :",
    "Post max size:" : "Taille maximale des données POST :",
    "SAPI:" : "SAPI :",
    "Extensions:" : "Extensions :",
    "PHP Info:" : "PHP Info :",
    "Show phpinfo" : "Afficher phpinfo",
    "FPM worker pool" : "pool du worker FPM",
    "Pool name:" : "Nom du pool :",
    "Pool type:" : "Type de pool :",
    "Start time:" : "Heure de début :",
    "Accepted connections:" : "Connexions acceptées :",
    "Total processes:" : "Nombre total de processus :",
    "Active processes:" : "Processus actifs :",
    "Idle processes:" : "Processus inactifs :",
    "Listen queue:" : "File d'attente d'écoute :",
    "Slow requests:" : "Requêtes lentes :",
    "Max listen queue:" : "File d'attente d'écoute maximale :",
    "Max active processes:" : "Nombre maximal de processus actifs :",
    "Max children reached:" : "Nombre maximal d'enfants atteint :",
    "CPU" : "CPU",
    "Swap" : "SWAP",
    "Resource usage" : "Utilisation des ressources",
    "Resource usage not available" : "Utilisation des ressources non disponible",
    "Monitoring pages" : "Pages de surveillance",
    "Shares" : "Partages",
    "Users:" : "Utilisateurs :",
    "Groups:" : "Groupes :",
    "Links:" : "Liens :",
    "Emails:" : "E-mails :",
    "Federated sent:" : "Offre de fédération envoyée :",
    "Federated received:" : "Offre de fédération reçue :",
    "Talk conversations:" : "Conversations Talk :",
    "Runs" : "Exécutions",
    "Average" : "Moyenne",
    "Longest" : "Durée maximale",
    "Critical" : "Critique",
    "Operating System:" : "Système d’exploitation :",
    "CPU:" : "CPU :",
    "{name} ({threads} threads)" : "{name} ({threads} fils d’exécution)",
    "Server time:" : "Heure du serveur :",
    "Uptime:" : "Durée de fonctionnement :",
    "Temperature" : "Température",
    "All" : "Tous",
    "The database checks could not be run." : "Les vérifications de la base de données n'ont pas pu être exécutées.",
    "Database checks" : "Vérifications de la base de données",
    "Database connection settings" : "Paramètres de connexion à la base de données",
    "These checks only cover MySQL, MariaDB and PostgreSQL. This server uses a different database." : "Ces vérifications ne couvrent que MySQL, MariaDB et PostgreSQL. Ce serveur utilise une base de données différente.",
    "Filter checks by status" : "Filtrer les vérifications par statut",
    "No checks match this filter." : "Aucune vérification ne correspond à ce filtre.",
    "_%n failing_::_%n failing_" : ["%n échoue","%n échouent","%n échouent"],
    "The server information could not be loaded." : "Les informations du serveur n'ont pas pu être chargées.",
    "Try again" : "Réessayer",
    "These values stopped updating at {time}: the server is not responding." : "Ces valeurs ont arrêté d'être mises à jour à {time} : le serveur ne répond pas.",
    "The live values could not be loaded: the server is not responding." : "Les valeurs actuelles n'ont pas pu être chargées : le serveur ne répond pas.",
    "{duration} ms" : "{duration} ms",
    "{duration} s" : "{duration} s",
    "CPU Usage:" : "Utilisation CPU :",
    "Load average: {percentage} % ({load}) last minute" : "Charge moyenne : {percentage}% ( {load}) dernière minute",
    "{lastMinutePercentage} % ({lastMinute}) last Minute\n{last5MinutesPercentage} % ({last5Minutes}) last 5 Minutes\n{last15MinutesPercentage} % ({last15Minutes}) last 15 Minutes" : "{lastMinutePercentage} % ({lastMinute}) la dernière minute\n{last5MinutesPercentage} % ({last5Minutes}) les 5 dernières minutes\n{last15MinutesPercentage} % ({last15Minutes}) les 15 dernières minutes",
    "RAM Usage:" : "Utilisation RAM :",
    "SWAP Usage:" : "Utilisation SWAP :",
    "RAM: Total: {memTotalBytes}/Current usage: {memUsageBytes}" : "RAM : Total : {memTotalBytes} / Utilisation actuelle : {memUsageBytes}",
    "SWAP: Total: {swapTotalBytes}/Current usage: {swapUsageBytes}" : "SWAP : Total : {swapTotalBytes} / Utilisation actuelle : {swapUsageBytes}",
    "SWAP info not available" : "Informations SWAP non disponibles",
    "Copied!" : "Copié !",
    "Not supported!" : "Non supporté !",
    "Press ⌘-C to copy." : "Appuyer sur ⌘-C pour copier.",
    "Press Ctrl-C to copy." : "Appuyez sur Ctrl-C pour copier.",
    "threads" : "fil de discussion",
    "Memory:" : "Mémoire :",
    "Files:" : "Fichiers :",
    "Storages:" : "Stockages :",
    "Free Space:" : "Espace libre :",
    "Hostname:" : "Nom d'hôte :",
    "Gateway:" : "Passerelle :",
    "%s%% of all users" : "%s%% pour tous les utilisateurs",
    "Memory limit:" : "Limite de mémoire :",
    "MB" : "Mo",
    "OPcache Revalidate Frequency:" : "Fréquence de revalidation de l'OPcache :",
    "External monitoring tool" : "Outil de surveillance externe",
    "Use this end point to connect an external monitoring tool:" : "Utiliser ce point de terminaison pour connecter un outil de surveillance externe :",
    "Copy" : "Copier",
    "Skip apps section (including apps section will send an external request to the app store)" : "Ignorer la section des applications (y compris la section des applications enverra une demande externe à l'App Store)",
    "To use an access token, please generate one then set it using the following command:" : "Pour utiliser un jeton d'accès, veuillez en générer un puis le définir à l'aide de la commande suivante :",
    "Then pass the token with the \"NC-Token\" header when querying the above URL." : "Puis transmettez le jeton avec l’entête « NC-Token » lorsque vous appelez l’URL.",
    "%1$s (%2$d threads)" : "%1$s (%2$d fils d’exécution)",
    "DNS:" : "DNS :"
},
"nplurals=3; plural=(n == 0 || n == 1) ? 0 : n != 0 && n % 1000000 == 0 ? 1 : 2;");
